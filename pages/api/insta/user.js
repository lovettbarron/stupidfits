...
const getUserData = (username) => {
  return new Promise(async (resolve, reject) => {
    ...
    const GQL = await axios(REQUEST_PARAMETERS).catch(reject);
    if (GQL) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (GQL) {
      ...
      resolve({
        ...
        caption: has_caption
          ? media_data.edge_media_to_caption.edges[0].node.text
          : null,
        ...
        childrenPictures: media_data.edge_sidecar_to_children
          ? media_data.edge_sidecar_to_children.map((edge) => {
              return {
                id: edge.node.id,
                shortcode: edge.node.shortcode,
                dimensions: edge.node.dimensions,
        ...
        taggedUsers: media_data.edge_media_to_tagged_user.edges
          ? media_data.edge_media_to_tagged_user.edges.map((tag) => {
              return {
